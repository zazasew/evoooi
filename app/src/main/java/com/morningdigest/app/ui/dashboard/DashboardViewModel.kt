...
class DashboardViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun refreshPolice() {
        viewModelScope.launch {
            ...
            _uiState.value = _uiState.value.copy(isRefreshingPolice = true, policeMunicipality = settings.policeMunicipality)
            try {
                val dismissed = container.settingsRepository.getPoliceDismissedIds()
                val incidents = container.policeIncidentFetcher.fetch(settings.policeMunicipalities, settings.policeCategories)
